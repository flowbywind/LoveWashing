...
//	 = -120
	...
	public int mMarginDelta = -60;
	private int dp2Pixel() {
		float denisty = getResources().getDisplayMetrics().density;
		mMarginDelta = (int) (mMarginDelta * denisty);
		return mMarginDelta;
	}
	...
	private void setLastRefreshTimeText() {
		...
		if (refreshListener != null) {
			refreshListener.onRefreshState(true);
		}
	...
	private void returnInitState() {
		...
		invalidate();
		if (refreshListener != null) {
			refreshListener.onRefreshState(false);
		}
	}
	...
	private void doMovement(int moveY) {
		...
		if (lp.topMargin > 0) {
			...
			refreshIndicatorView.setImageResource(R.drawable.z_arrow_up);
		} else {
	...
	public void finishRefresh() {
		Log.i(TAG, "执行了=====finishRefresh   refreshTargetTop="
				+ refreshTargetTop);
		LinearLayout.LayoutParams lp = (LinearLayout.LayoutParams) this.refreshView
				.getLayoutParams();
		int i = lp.topMargin;
	...
	private boolean canScroll() {
		...
		if (getChildCount() > 1) {
			...
			if (childView instanceof ListView) {
				if(childView!=null&&((ListView) childView).getChildAt(0)!=null){
	...
	public interface RefreshListener {
		...
		public void onRefreshState(boolean isXiala);
	}
