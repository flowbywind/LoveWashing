...
import com.easylife.app_wash.widget.ScrollOverListView.OnScrollOverListener;
...
public class PullDownView extends LinearLayout implements OnScrollOverListener {
	...
	private static int DEFAULT_HEADER_VIEW_HEIGHT = 105; // 头部文件原本的高度
	...
	private void initHeaderViewAndFooterViewAndListView(Context context) {
		...
		mContext = context;
		// DEFAULT_HEADER_VIEW_HEIGHT = (int) dip2px(mContext, 70f);
		...
		mFooterView.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if (!mIsFetchMoreing) {
					...
					mFooterLoadingView.setVisibility(View.VISIBLE);
					mFooterTextView.setText("正在加载,请稍后......");
		...
		mListView.setDivider(context.getResources()
				.getDrawable(R.drawable.line_horizental));
	...
	public boolean onMotionUp(MotionEvent ev) {
		...
		if (mHeaderViewParams.height > 0) {
			...
			// 此处做修改 为防止下拉刷新时 item停留在pressed 状态 故此处return 返回 false 不消耗up事件
			// 未避免触发点击事件 此处增加标识量 如果isRefresh 为true 则不触发 点击事件
			isRefresh = true;
			return false;
		}
		isRefresh = false;
	...
	public static float dip2px(Context context, float dpValue) {
		...
		Log.d("zhangyaobin", "scale=" + scale + "   dpValue * scale=" + dpValue
				* scale + "  dpi=" + dpi);
